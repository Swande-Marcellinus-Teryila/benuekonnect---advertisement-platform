	<?php 
					 ...
					 $total_vacancies = $crud->getTotal('Vacancy');
							if (isset($_GET['getlm'])) {
								$limit=$_GET['getlm'];
								if($total_vacancies-$limit>50){
									$limit+=50;
								}else{
									$limit+=$total_vacancies%$limit;
								}
							}
						$latestVacancys = $crud->displayAllSpecificWithOrderWithLimit2('vacancy', 'status', 'user_approval_status', '1', '1', 'date_uploaded', 'DESC', $limit);
						$Vacancy = "";
						$photo="";
					...
					?>
							<div class="mob-adverts" id="id<?=$idcounter?>">
								<span class="text-blue"><i class="fa fa-clock-o"></i> 
									<?=$crud->get_time_ago($lvacancy['date_uploaded']);?>
								...
								<table cellspacing="0" border="10px" class="category_table" width="100">
									...
									<tr>
										<td rowspan="5" width="30%">
											<img src="users/profile_picture/<?= $photo ?>"id="big_td_img2"></td>
									...
									<tr><td colspan="2" style="height:25px" >
										<strong class="stage_name"><?=$lvacancy['stage_name']?></strong></td>
									</tr>
											</table><br>
